...
| Phase 50 (v0.9.92)  | Keyboard navigation (Ctrl+Shift+1..9 / 0 / D), a11y landmarks + live region for collapse/detach announcements + resize-handle keyboard support, Phoenix telemetry hooks for dock-pane lifecycle (gated on `telemetry.phoenix.enabled`). Dock root is now `<aside role="complementary" aria-label="App dock">`. Each pane is `<section role="region" aria-label={appName}>`. Hidden `aria-live="polite"` region announces collapse, expand, detach, reattach events. Resize handles have `role="separator"`, `tabindex="0"`, and arrow-key step handlers. Pane headers have `tabindex="0"` and `data-pane-index` for keyboard focus targeting. Defers lazy iframe mount + iframe pool to Phase 50b.                                                                                                                                                                                                                   |
| Phase 50b (v0.9.93) | Lazy iframe mount + iframe pool (Sub-A + Sub-B). Sub-A: three-signal keep-alive (`shouldKeepIframeAlive` — built-in server / observed state-relay / `alwaysLoaded` config flag). Non-keep-alive apps collapse via `<Show>` gate (iframe unmounts) instead of `display:none`; keep-alive apps still use `display:none`. State-relay saves (`mcp-app-state:save`) observed via `onSaveObserved` callback → `markRelayObserved(uri)` in `AppDockProvider`. Config key `mcp_apps: Record<uri, { alwaysLoaded?: boolean }>` added to Zod schema + SDK type. "Always keep loaded" checkbox in pane ⋮ menu (non-builtin apps only) with optimistic config update + rollback. One-shot toast on first unknown-app collapse. Sub-B: `IframePool` (max 3, 5-min TTL, LRU eviction, off-screen host div) parks iframes on `dock.remove()` for quick re-pin without cold-start handshake. Pool claim path deferred to Phase 50c. |
