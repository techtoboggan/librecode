...
test("changing theme persists in localStorage", async ({ page, gotoSession }) => {
  ...
  const storedThemeId = await page.evaluate(() => {
    return localStorage.getItem("librecode-theme-id")
  })
...
test("legacy oc-1 theme migrates to oc-2", async ({ page, gotoSession }) => {
  ...
  await expect
    .poll(async () => {
      return await page.evaluate(() => {
        return localStorage.getItem("librecode-theme-css-light")
      })
    })
    .toBeNull()
  ...
  await expect
    .poll(async () => {
      return await page.evaluate(() => {
        return localStorage.getItem("librecode-theme-css-dark")
      })
    })
    .toBeNull()
})
