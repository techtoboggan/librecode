...
const shareDisabled = process.env.LIBRECODE_DISABLE_SHARE === "true" || process.env.LIBRECODE_DISABLE_SHARE === "1"
...
test("/share and /unshare update session share state", async ({ page, sdk, gotoSession }) => {
  test.skip(shareDisabled, "Share is disabled in this environment (LIBRECODE_DISABLE_SHARE).")
