...
class MemoryStorage implements Storage {
  ...
  getItem(key: string) {
    ...
    if (key.startsWith("librecode.throw")) throw new Error("storage get failed")
  ...
  removeItem(key: string) {
    ...
    if (key.startsWith("librecode.throw")) throw new Error("storage remove failed")
...
describe("persist localStorage resilience", () => {
  ...
  test("disables only the failing scope when storage throws", () => {
    const bad = persistTesting.localStorageWithPrefix("librecode.throw.scope")
  ...
  test("failing fallback scope does not poison direct storage scope", () => {
    const broken = persistTesting.localStorageWithPrefix("librecode.throw.scope2")
