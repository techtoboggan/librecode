...
async function writeManagedSettings(settings: object, filename = "librecode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "librecode.json") {
...
test("ignores legacy tui keys in librecode config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://librecode.app/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "librecode.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  Account.config = mock(async () => ({
    provider: {
      librecode: {
        options: {
          apiKey: "{env:LIBRECODE_CONSOLE_TOKEN}",
        },
      },
    },
  }))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "librecode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .librecode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const librecodeDir = path.join(dir, ".librecode")
      await fs.mkdir(librecodeDir, { recursive: true })
      const agentDir = path.join(librecodeDir, "agent")
...
test("loads agents from .librecode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const librecodeDir = path.join(dir, ".librecode")
      await fs.mkdir(librecodeDir, { recursive: true })

      const agentsDir = path.join(librecodeDir, "agents")
...
test("loads commands from .librecode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const librecodeDir = path.join(dir, ".librecode")
      await fs.mkdir(librecodeDir, { recursive: true })

      const commandDir = path.join(librecodeDir, "command")
...
test("loads commands from .librecode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const librecodeDir = path.join(dir, ".librecode")
      await fs.mkdir(librecodeDir, { recursive: true })

      const commandsDir = path.join(librecodeDir, "commands")
...
test("does not try to install dependencies in read-only LIBRECODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
...
test("installs dependencies in writable LIBRECODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "librecode.json"),
        JSON.stringify({ $schema: "https://librecode.app/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .librecode config
      const projectDir = path.join(dir, "project")
      const librecodeDir = path.join(projectDir, ".librecode")
      await fs.mkdir(librecodeDir, { recursive: true })
      ...
      // Local .librecode config with different plugins
      await Filesystem.write(
        path.join(librecodeDir, "librecode.json"),
        JSON.stringify({
          $schema: "https://librecode.app/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .librecode config with some overlapping plugins
      await Filesystem.write(
        path.join(librecodeDir, "librecode.json"),
        JSON.stringify({
          $schema: "https://librecode.app/config.json",
...
// Note: preload.ts sets LIBRECODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .librecode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .librecode directory config enables it
      const librecodeDir = path.join(dir, ".librecode")
      await fs.mkdir(librecodeDir, { recursive: true })
      await Filesystem.write(
        path.join(librecodeDir, "librecode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/librecode")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/librecode")
...
describe("getPluginName", () => {
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@librecode/plugin@2.0.0")).toBe("@librecode/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("local plugin directory overrides global librecode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const librecodeDir = path.join(projectDir, ".librecode")
        const pluginDir = path.join(librecodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("LIBRECODE_DISABLE_PROJECT_CONFIG", () => {
  ...
  test("skips project .librecode/ directories when flag is set", async () => {
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .librecode directory with a command
          const librecodeDir = path.join(dir, ".librecode", "command")
          await fs.mkdir(librecodeDir, { recursive: true })
          await Filesystem.write(path.join(librecodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
  ...
  test("LIBRECODE_CONFIG_DIR still works when flag is set", async () => {
    ...
    try {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          const config = await Config.get()
          // Should load from LIBRECODE_CONFIG_DIR, not project
