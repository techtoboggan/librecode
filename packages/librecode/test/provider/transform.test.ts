...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const librecodeModel = {
      ...openaiModel,
      providerID: "librecode",
      api: {
        id: "librecode-test",
        url: "https://api.librecode.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    expect(result[0].content[0].providerOptions?.librecode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          librecode: { itemId: "msg_librecode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              librecode: { itemId: "msg_librecode_part" },
    ...
    expect(result[0].providerOptions?.librecode?.itemId).toBe("msg_librecode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.librecode?.itemId).toBe("msg_librecode_part")
