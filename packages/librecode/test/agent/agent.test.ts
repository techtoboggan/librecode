...
import { Agent, type AgentInfo } from "../../src/agent/agent"
import { PermissionNext } from "../../src/permission/next"
import { Instance } from "../../src/project/instance"
import { tmpdir } from "../fixture/fixture"

// Helper to evaluate permission for a tool with wildcard pattern
function evalPerm(agent: AgentInfo | undefined, permission: string): PermissionNext.Action | undefined {
...
test("plan agent denies edits except .librecode/plans/*", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
...
test("explicit Truncate.GLOB deny is respected", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const build = await Agent.get("build")
      const buildAgent = build as NonNullable<typeof build>
      expect(PermissionNext.evaluate("external_directory", Truncate.GLOB, buildAgent.permission).action).toBe("deny")
      expect(PermissionNext.evaluate("external_directory", Truncate.DIR, buildAgent.permission).action).toBe("deny")
    },
  })
})
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".librecode", "skill", "perm-skill")
      await Bun.write(
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const build = await Agent.get("build")
        const buildAgent = build as NonNullable<typeof build>
        const skillDir = path.join(tmp.path, ".librecode", "skill", "perm-skill")
        const target = path.join(skillDir, "reference", "notes.md")
        expect(PermissionNext.evaluate("external_directory", target, buildAgent.permission).action).toBe("allow")
      },
    })
  } finally {
