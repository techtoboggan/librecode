...
const _fileLog = Log.create({ service: "file" })
...
type FileNodeType = z.infer<typeof FileNode>
...
type FileContentType = z.infer<typeof FileContent>
...
async function shouldEncode(mimeType: string): Promise<boolean> {
  ...
  _fileLog.info("shouldEncode", { type })
...
const _fileState = Instance.state(async () => {
...
async function collectModifiedFiles(): Promise<FileInfoType[]> {
  const diffOutput = (
...
async function collectAddedFiles(): Promise<FileInfoType[]> {
...
async function collectDeletedFiles(): Promise<FileInfoType[]> {
  const deletedOutput = (
    await git(
...
async function fileStatus(): Promise<FileInfoType[]> {
...
async function readImageFile(file: string, full: string): Promise<FileContentType> {
...
async function readTextWithDiff(file: string, _full: string, content: string): Promise<FileContentType> {
...
async function fileRead(file: string): Promise<FileContentType> {
  using _ = _fileLog.time("read", { file })
...
function nodeSortComparator(a: FileNodeType, b: FileNodeType): number {
  if (a.type !== b.type) return a.type === "directory" ? -1 : 1
  return a.name.localeCompare(b.name)
}

async function fileList(dir?: string): Promise<FileNodeType[]> {
...
async function fileSearch(input: {
  ...
  _fileLog.info("search", { query, kind })

  const result = await _fileState().then((x) => x.files())
  ...
  _fileLog.info("search", { query, kind, results: output.length })
  return output
}
...
export declare namespace File {
  type Info = FileInfoType
  type Node = FileNodeType
  type Content = FileContentType
}
