...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for librecode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal librecode sessions
...
librecode acp --cwd /path/to/project
```
...
```bash
LIBRECODE_ENABLE_QUESTION_TOOL=1 librecode acp
```
...
- **Terminal Integration**: Full terminal support via librecode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | librecode acp
```
...
### Mapping to LibreCode

ACP sessions map cleanly to librecode's internal session model:
