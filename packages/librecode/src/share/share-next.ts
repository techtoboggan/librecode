...
async function shareNextUrl() {
  const req = await shareNextRequest()
  return req.baseUrl
}

async function shareNextRequest(): Promise<{
...
async function shareNextCreate(sessionID: SessionID) {
...
async function shareNextRemove(sessionID: SessionID) {
  if (disabled) return
...
export const ShareNext = {
  url: shareNextUrl,
  request: shareNextRequest,
  init: shareNextInit,
  create: shareNextCreate,
  remove: shareNextRemove,
} as const
