...
export async function sessionCompactionPrune(input: { sessionID: SessionID }): Promise<void> {
...
export async function sessionCompactionIsOverflow(input: {
...
export async function sessionCompactionProcess(input: {
  parentID: MessageID
  messages: MessageV2.WithParts[]
  sessionID: SessionID
  abort: AbortSignal
  ...
  Bus.publish(sessionCompactionEvent.Compacted, { sessionID: input.sessionID })
  return "continue"
}

export const sessionCompactionCreate = fn(
  z.object({
    sessionID: SessionID.zod,
    agent: z.string(),
    model: z.object({
...
export const SessionCompaction = {
  ...
  isOverflow: sessionCompactionIsOverflow,
  prune: sessionCompactionPrune,
  process: sessionCompactionProcess,
  create: sessionCompactionCreate,
  PRUNE_MINIMUM,
  PRUNE_PROTECT,
} as const
