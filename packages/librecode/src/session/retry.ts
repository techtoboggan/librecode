...
async function sessionRetrySleep(ms: number, signal: AbortSignal): Promise<void> {
...
function sessionRetryDelay(attempt: number, error?: MessageV2.APIError): number {
  if (error) {
...
function sessionRetryRetryable(error: ReturnType<NamedError["toObject"]>): string | undefined {
...
export const SessionRetry = {
  ...
  RETRY_MAX_DELAY,
  sleep: sessionRetrySleep,
  delay: sessionRetryDelay,
  retryable: sessionRetryRetryable,
} as const
