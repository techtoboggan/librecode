...
function applyAgentScalarFields(item: AgentInfo, value: AgentConfig): void {
...
function applyAgentRemainingFields(item: AgentInfo, value: AgentConfig): void {
...
function applyUserAgentConfig(item: AgentInfo, value: AgentConfig): void {
  applyAgentScalarFields(item, value)
  applyAgentRemainingFields(item, value)
}

function ensureTruncateGlobAllowed(result: Record<string, AgentInfo>): void {
...
const state = Instance.state(async () => {
  ...
  const result: Record<string, AgentInfo> = buildNativeAgents(defaults, user, whitelistedDirs)
...
export async function agentGet(agent: string): Promise<AgentInfo | undefined> {
...
export async function agentDefaultAgent(): Promise<string> {
  const cfg = await Config.get()
...
export async function agentGenerate(input: {
  description: string
  ...
  const existing = await agentList()
...
export const Agent = {
  ...
  get: agentGet,
  list: agentList,
  defaultAgent: agentDefaultAgent,
