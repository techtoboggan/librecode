...
function transformMessage(msgs: ModelMessage[], model: Provider.Model, options: Record<string, unknown>) {
...
function transformTemperature(model: Provider.Model) {
...
function transformTopP(model: Provider.Model) {
  const id = model.id.toLowerCase()
...
function transformTopK(model: Provider.Model) {
  const id = model.id.toLowerCase()
...
function transformVariants(model: Provider.Model): Record<string, Record<string, unknown>> {
...
function transformOptions(input: {
  model: Provider.Model
  sessionID: string
...
function transformSmallOptions(model: Provider.Model) {
  if (
...
function transformProviderOptions(model: Provider.Model, options: { [x: string]: unknown }): SharedV2ProviderOptions {
...
function transformMaxOutputTokens(model: Provider.Model): number {
  return Math.min(model.limit.output, OUTPUT_TOKEN_MAX) || OUTPUT_TOKEN_MAX
}

function transformSchema(model: Provider.Model, schema: JSONSchema.BaseSchema | JSONSchema7): JSONSchema7 {
  /*
...
export const ProviderTransform = {
  ...
  smallOptions: transformSmallOptions,
  providerOptions: transformProviderOptions,
  maxOutputTokens: transformMaxOutputTokens,
