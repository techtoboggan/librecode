...
const InitializeError = NamedError.create(
  "LSPInitializeError",
  z.object({
    serverID: z.string(),
  }),
...
async function create(input: { serverID: string; server: LSPServer.Handle; root: string }) {
  ...
  const diagnostics = new Map<string, LSPClientDiagnostic[]>()
...
export declare namespace LSPClient {
  ...
  type Diagnostic = LSPClientDiagnostic
}
