...
async function hasClients(file: string) {
  const s = await state()
  const extension = path.parse(file).ext || file
  for (const server of Object.values(s.servers)) {
...
async function touchFile(input: string, waitForDiagnostics?: boolean) {
...
async function documentSymbol(uri: string) {
  ...
  return run(file, (client) =>
    ...
    .then((result) => result.flat() as (LSPDocumentSymbol | LSPSymbolInfo)[])
...
async function implementation(input: { file: string; line: number; character: number }) {
  return run(input.file, (client) =>
    client.connection
...
async function prepareCallHierarchy(input: { file: string; line: number; character: number }) {
  return run(input.file, (client) =>
    client.connection
...
async function outgoingCalls(input: { file: string; line: number; character: number }) {
  return run(input.file, async (client) => {
    const items = (await client.connection
      .sendRequest("textDocument/prepareCallHierarchy", {
        textDocument: { uri: pathToFileURL(input.file).href },
...
function diagnosticPretty(diagnostic: LSPClient.Diagnostic) {
  const severityMap = {
    1: "ERROR",
    2: "WARN",
    3: "INFO",
...
export type LSPRange = z.infer<typeof Range>
export type LSPSymbolInfo = z.infer<typeof SymbolInfo>
export type LSPDocumentSymbol = z.infer<typeof DocumentSymbol>
export type LSPStatus = z.infer<typeof Status>
...
export declare namespace LSP {
  type Range = LSPRange
  type SymbolInfo = LSPSymbolInfo
  type DocumentSymbol = LSPDocumentSymbol
  type Status = LSPStatus
}
