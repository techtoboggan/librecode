      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for librecode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found librecode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["librecode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        // Launch librecode TUI with session ID if available
        const { spawn } = await import("child_process")
        const librecodeArgs = sessionId ? ["-s", sessionId] : []
        const librecodeProcess = spawn("librecode", librecodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })
