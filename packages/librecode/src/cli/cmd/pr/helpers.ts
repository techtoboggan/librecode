...
export async function checkoutPrBranch(prNumber: number, localBranchName: string): Promise<boolean> {
  ...
  const result = await Process.run(["gh", "pr", "checkout", `${prNumber}`, "--branch", localBranchName, "--force"], {
    nothrow: true,
  })
  if (result.code !== 0) {
...
export async function configureForkRemote(prInfo: Record<string, unknown>, localBranchName: string): Promise<void> {
