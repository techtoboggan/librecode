...
const patchLog = Log.create({ service: "patch" })
...
export const ApplyPatchError = {
  ParseError: "ParseError",
  IoError: "IoError",
  ComputeReplacements: "ComputeReplacements",
  ImplicitInvocation: "ImplicitInvocation",
} as const
export type ApplyPatchError = (typeof ApplyPatchError)[keyof typeof ApplyPatchError]
...
export type MaybeApplyPatch = (typeof MaybeApplyPatch)[keyof typeof MaybeApplyPatch]
...
export type MaybeApplyPatchVerified = (typeof MaybeApplyPatchVerified)[keyof typeof MaybeApplyPatchVerified]
...
export function maybeParseApplyPatch(
  ...
  | { type: typeof MaybeApplyPatch.Body; args: ApplyPatchArgs }
  | { type: typeof MaybeApplyPatch.PatchParseError; error: Error }
  | { type: typeof MaybeApplyPatch.NotApplyPatch } {
...
async function applyAddHunk(hunk: Extract<PatchHunk, { type: "add" }>, added: string[]): Promise<void> {
  ...
  patchLog.info(`Added file: ${hunk.path}`)
}

// Apply a single "delete" hunk to the filesystem
async function applyDeleteHunk(hunk: Extract<PatchHunk, { type: "delete" }>, deleted: string[]): Promise<void> {
  await fs.unlink(hunk.path)
  deleted.push(hunk.path)
  patchLog.info(`Deleted file: ${hunk.path}`)
}

// Apply a single "update" hunk to the filesystem (handles move_path)
async function applyUpdateHunk(hunk: Extract<PatchHunk, { type: "update" }>, modified: string[]): Promise<void> {
  ...
  if (hunk.move_path) {
    ...
    patchLog.info(`Moved file: ${hunk.path} -> ${hunk.move_path}`)
  } else {
    await fs.writeFile(hunk.path, fileUpdate.content, "utf-8")
    modified.push(hunk.path)
    patchLog.info(`Updated file: ${hunk.path}`)
  }
}

// Apply hunks to filesystem
export async function applyHunksToFiles(hunks: PatchHunk[]): Promise<AffectedPaths> {
  if (hunks.length === 0) {
...
async function buildDeleteChange(
  hunk: Extract<PatchHunk, { type: "delete" }>,
  effectiveCwd: string,
...
async function resolveHunkChange(hunk: PatchHunk, effectiveCwd: string): Promise<HunkChangeResult> {
  if (hunk.type === "add") {
    return {
      ok: true,
...
export async function maybeParseApplyPatchVerified(
  ...
  | { type: typeof MaybeApplyPatchVerified.Body; action: ApplyPatchAction }
  | { type: typeof MaybeApplyPatchVerified.CorrectnessError; error: Error }
  | { type: typeof MaybeApplyPatchVerified.NotApplyPatch }
> {
