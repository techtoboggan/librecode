...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("librecode-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("librecode_lib=debug,librecode_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("librecode_lib=info,librecode_desktop=info,sidecar=info")
        }
    });
