...
- **File length**: Maximum 1,000 lines. Files over 500 should be evaluated for splitting.
  - **Documented exceptions** (exempt from the rule, header comment required):
    1. **Codegen output** — files whose entire content is regenerated by a build script (e.g. `packages/ui/src/components/file-icons/types.ts` from the icon spritesheet generator, `packages/sdk/js/src/v2/gen/types.gen.ts` from the OpenAPI generator). Splitting either lies to the generator or creates a pointless barrel.
    2. **Vendored upstream code** — files copied from another project's source tree where splitting would diverge from upstream's layout and make future merges painful (e.g. `packages/librecode/src/provider/sdk/copilot/responses/openai-responses-language-model.ts` from `@ai-sdk/openai`).
    3. **Tightly-shared private state** — modules where the only available split would either expose private state through the public barrel or introduce a `_shared.ts` indirection that doesn't actually reduce coupling (e.g. `packages/librecode/src/mcp/index.ts` — the OAuth flow + transport creation share `state()`, `pendingOAuthTransports`, and `create()`). These should be split when the next real refactor of the same area lands, not gratuitously.
  - Files over 1,000 lines that don't fall into one of the above MUST split. Add a header comment to each exception explaining which category it falls under.
