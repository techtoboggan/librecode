...
### Phase 56: Dock & Activity Graph UX fixes ✅

Three issues Tristan hit in daily App Dock use.

| Fix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Status |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------ |
| **Activity Graph readable at scale** — the agent-phase comma-wall (`completed, completed, …`) wrapped and buried the canvas. Replaced with a two-tier glanceable header: live-focus line (`✎ <file>` / `N working` / `Idle`) + cumulative outcomes (`N done · N active · N error`, errors in danger color). Label pinned single-line; canvas labels thinned >24 nodes, radius clamped >50. `summarizeActivity()` in `fs-activity-graph.html`. State-of-the-art grounded (Dibia / agentic-design.ai / DiLLS). | ✅     |
| **De-duplicated dock title** — McpAppPanel inner header suppressed when embedded-inline (PaneHeader already shows the name); kept for detached + fullscreen/overlay + overlay-capable apps' ⤢ button. `shouldShowInnerHeader()` in `mcp-app-panel/chrome.ts`.                                                                                                                                                                                                                                                | ✅     |
| **Disconnect → ⋮ kebab** — moved from the removed inner header into PaneMenu (above Remove; distinct: drops session grants + closes bridge, keeps the pane). Disconnect fn surfaced from DockPane via a signal.                                                                                                                                                                                                                                                                                              | ✅     |
| **Dock space reserved** — review panel floored at `max(360px, calc(100% − session − dock))` so opening the dock can't collapse it; the flex-initial session panel yields, the shrink-0 dock stays on-screen. Shared `dockOffsetWidth()` helper.                                                                                                                                                                                                                                                              | ✅     |
| Regression tests: L1 — `activity-summary`, `mcp-app-panel/chrome`, `pane-menu` showDisconnect, `state` dockOffsetWidth, builtin-apps source guards. L3 — `app-dock.spec` (name-once + Disconnect-in-menu), `dock-fits.spec` (1100px review width).                                                                                                                                                                                                                                                           | ✅     |
